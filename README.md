...
- 일정 클릭 시 설명/장소 펼쳐보기 — Google Calendar 웹에서 작성한 서식(굵게, 목록, 줄바꿈, 링크)도 그대로 보이고, 링크는 클릭하면 기본 브라우저로 열립니다
- **일정 제목 우클릭 → 수정/삭제**, **펼친 설명란은 클릭하면 바로 수정** (내 캘린더 일정만 가능, 공휴일 항목은 읽기 전용)
- **"+" 버튼으로 일정 추가** — 메모장과 같은 팝업(제목 + 날짜 범위 + 종일/시간)으로 바로 생성. Google 캘린더 웹은 하단의 "Google 캘린더에서 열기" 링크로 별도 이동
- 20분마다 자동 새로고침, 수동 새로고침 버튼. 위젯 자체에서 일정을 수정/삭제/추가하거나 메모장에서 "일정에 추가"를 해도 그 자리에서 자동으로 새로고침되고, 자정이 지나면 오늘/내일 데이터도 자동으로 다시 불러옵니다
...
삭제 확인창과 "일정 추가" 팝업은 위젯과 메모장 두 창이 같은 컴포넌트를 공유해서 디자인이 통일돼 있습니다.
...
5. **테스트 사용자(Test users)** 단계에서 본인의 Gmail 주소를 추가
   ...
   - 앱을 "프로덕션(게시)" 상태로 전환하는 것은 지금은 권장하지 않습니다 — Google의 검증 절차(무료지만 며칠~몇 주 소요)가 필요합니다. 개인용으로는 테스트 사용자 추가만으로 충분하지만, 그 대가로 **로그인 토큰이 7일마다 만료**됩니다 (아래 "토큰 만료와 자동 재로그인" 참고).
...
OAuth 동의 화면이 "테스트" 상태인 동안, Google은 로그인 토큰(refresh token)을 **7일 후 자동 만료**시킵니다. 만료되면 캘린더/메모 관련 요청이 전부 `invalid_grant` 오류로 실패합니다.

앱이 이 상황을 스스로 감지해서 처리합니다: API 요청이 `invalid_grant`로 실패하면, 죽은 토큰을 지우고 자동으로 로그인 창을 다시 띄운 뒤 요청을 재시도합니다. 즉 실제로 겪는 건 **대략 일주일에 한 번, 위젯을 켰을 때 브라우저 로그인 창이 잠깐 떴다 사라지는 것**뿐입니다 — 로그인/동의만 다시 해주면 나머지는 자동으로 이어집니다.

토큰이 죽어도 캘린더/Drive의 실제 데이터(일정, 메모)는 전혀 영향받지 않습니다 — 그저 앱이 잠깐 접근을 못 하는 것뿐입니다.
...
체감 속도를 위해 내부적으로 다음을 적용해뒀습니다 (설정할 것은 없습니다):

- Google API 클라이언트/인증 객체를 매 호출마다 새로 만들지 않고 재사용
- 공휴일 데이터는 잘 바뀌지 않으므로 같은 날짜 범위는 12시간 캐싱 (일/주/월 전환 시 내 캘린더만 새로 불러옴)
- HTTPS 연결 재사용(keep-alive)으로 매 요청마다 새로 연결하지 않도록 함
- 캘린더 조회 시 실제 쓰는 필드만 요청

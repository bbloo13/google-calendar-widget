...
- 커서가 있는 줄에 에디터 헤더의 **S** 버튼으로 취소선 켜기/끄기 (완료한 항목 표시용). 표준 마크다운(`~~텍스트~~`)으로 저장되므로 Drive 미리보기나 다른 마크다운 도구에서도 그대로 취소선으로 보입니다
- 메모에 파일 첨부 — 편집창에 파일을 드래그하거나 이미지를 붙여넣기(Ctrl+V)하면 자동 업로드. 사진은 클릭 시 미리보기/다운로드 선택, 그 외 파일은 클릭 한 번으로 다운로드 폴더에 바로 저장
...
- 메모 편집창에 파일을 드래그하거나 이미지를 붙여넣으면 첨부파일로 업로드됩니다. Drive 상에서는 파일명이 아니라 `appProperties`에 심어둔 메모 파일 ID로 연결하기 때문에, 메모 제목이 겹치거나 나중에 바뀌어도 연결이 깨지지 않습니다. 첨부 목록은 편집창 상단에 아이콘+파일명 칩으로 표시되고, 칩에 마우스를 올리면 나오는 × 로 삭제할 수 있습니다. 사진은 클릭하면 "미리보기(OS 기본 이미지 뷰어)/다운로드" 중 고를 수 있고, 그 외 파일은 클릭 한 번으로 다운로드 폴더에 바로 저장됩니다 (저장 위치를 묻지 않음). 메모를 삭제하면 붙어있던 첨부파일도 함께 정리됩니다
...
- 메모장: 이번 세션에서 한 번 열어본 카테고리·메모는 다시 열 때 캐시로 즉시 표시되고, 그 뒤에 조용히 최신 데이터로 재확인됩니다 (stale-while-revalidate)
- 메모 자동 저장 반응 속도 개선 (입력 멈춘 뒤 저장 시작까지 800ms → 400ms)
- 위젯이 켜지자마자 메모장 백엔드(Drive 폴더 조회 등)를 미리 한 번 데워둬서, 메모장을 처음 열 때도 콜드 스타트 지연이 줄어듭니다
- 처음 불러오는 동안(캐시가 없을 때)에는 카테고리/메모 목록/메모 본문 헤더 아래에 각각 진행바가 표시됩니다
